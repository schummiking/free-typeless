...
if (-not (Test-Path (Join-Path $VendorDir "node_modules\electron-store"))) {
    Write-Host "[switch-account] Installing electron-store..."
    npm install --prefix $VendorDir --silent --no-fund --no-audit electron-store@10.0.1 | Out-Null
}

if (-not (Test-Path (Join-Path $VendorDir "node_modules\puppeteer"))) {
    Write-Host "[switch-account] Installing puppeteer (this may take a moment)..."
    npm install --prefix $VendorDir --silent --no-fund --no-audit puppeteer@24.2.1 | Out-Null
}
...
$env:NODE_PATH = Join-Path $VendorDir "node_modules"

node (Join-Path $ScriptDir "switch-account.mjs") @args
